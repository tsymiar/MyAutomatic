...
void EchoEngine::reverseFragment(int fragmentId)
{
    for (auto& f : m_fragments) {
        if (f.id == fragmentId) {
            // 首次逆向 = 怀疑声音 = 释放寄宿声灵（割舍声音，收集灵魂碎片）
            if (!f.isReversed) {
                releaseSpirit(fragmentSpirit(f.type));
            }
            f.isReversed = true;
            break;
        }
...
EndingType EchoEngine::calculateEnding() const
{
    ...
    if (ch3 == PlayerChoice::RejectFusion) {
        if (ch4 == PlayerChoice::TransferBack)
            return EndingType::SilentArchive;  // 拒绝融合 + 转回 → 救回林薇，却发现自己早已被忠诚声纹替换（静默）
        return EndingType::Matricide;          // 拒绝融合 + 删生命维持 → 亲手抹除林薇，成为新主任（弑母）
    }
    // 尝试分离（追求两全）
...
QString EchoEngine::chapterDescription() const
{
    ...
    case GameChapter::Prologue:
        return "2040年，你入职A-Lab的第7天，也是导师林薇失踪的第三天。她的工位上，耳机仍在循环那句语音日志：'别相信你听到的任何声音，包括你自己的。'\n你拿起她留下的声纹重构仪，抓取环境中的声波碎片、甄别它们的可信度——昨天下午茶水间的那段对话里，或许还藏着她消失前最后的声音。";
    case GameChapter::Chapter1:
        return "陈主任要你整理林薇失踪前72小时的全部工作录音，拼出一份'官方报告'。\n但你听得出来：那些被反复修剪、刻意抹平的碎片底下，藏着有人动过手脚的痕迹。";
    case GameChapter::Chapter2:
        return "你潜入地下三十米的废弃档案库，气温-4℃。这里的碎片不再是对话，而是纯粹的物理振动——金属的热胀冷缩、地磁的脉动，还有一颗被封进墙里的心跳。\n把它们与人体器官的频率对齐，被封印的历史将在共鸣中苏醒。";
    ...
    case GameChapter::Chapter4:
        return "废弃精神病院，地下三层。陈远山封锁了所有出口，也切断了每一条音频线路——他要让你这只'泡在静水里的耳朵'彻底失明。\n你的重构仪现在能主动发射频率：粉碎墙壁、伪造人声。只是每一次发射，都会在你身上留下更多无法抹去的声纹。";
    case GameChapter::Finale:
        return "最后一段音频就位。你终于明白——自己拼凑的从来不是一桩案件，而是一个被篡改过的人生。而你，正是那最后一个被替换的音节。";
    }
    return "";
}
...
// ═══════════════════════════════════════════
//  离奇魔幻基因：声灵系统
//  （每个声音碎片都寄宿着一只声灵，拼图即召唤）
// ═══════════════════════════════════════════

void EchoEngine::initSpirits()
{
    m_spirits = {
...
void EchoEngine::initSoulFragments()
{
...
SpiritInfo* EchoEngine::findSpirit(SpiritKind kind)
{
    for (auto& s : m_spirits) if (s.kind == kind) return &s;
    return nullptr;
}

// 碎片类型 → 寄宿声灵：让每种声音都对应一位声灵，拼图即唤醒
SpiritKind EchoEngine::fragmentSpirit(FragmentType type) const
{
    switch (type) {
    case FragmentType::Footstep:       return SpiritKind::Shattered;  // 背叛者的脚步
    case FragmentType::DoorCreak:      return SpiritKind::Shattered;  // 被推开的秘密之门
    case FragmentType::ElectricBuzz:   return SpiritKind::PrimeEcho;  // 电流嗡鸣
    case FragmentType::StaticNoise:    return SpiritKind::PrimeEcho;  // 白噪声
    case FragmentType::LowFrequency:   return SpiritKind::PrimeEcho;  // 低频嗡鸣
    case FragmentType::Geomagnetic:    return SpiritKind::PrimeEcho;  // 地磁波动
    case FragmentType::WaterDrop:      return SpiritKind::Mirror;     // 滴水反射
    case FragmentType::RainAmbient:    return SpiritKind::Mirror;     // 雨声倒映
    case FragmentType::ClockTick:      return SpiritKind::Mirror;     // 时钟的规律回声
    case FragmentType::Heartbeat:      return SpiritKind::Silence;    // 心跳与静默
    case FragmentType::Breath:         return SpiritKind::Silence;    // 呼吸渐止
    case FragmentType::VoiceWhisper:   return SpiritKind::Lie;        // 无法辨认的低语
    case FragmentType::Typewriter:     return SpiritKind::Lie;        // 打出来的都是伪证
    case FragmentType::TireScreech:    return SpiritKind::Wrath;      // 刺耳急刹
    case FragmentType::MetalExpansion: return SpiritKind::Wrath;      // 金属怒吼
    default:                           return SpiritKind::PrimeEcho;
    }
}

void EchoEngine::summonSpirit(SpiritKind kind)
{
    auto* s = findSpirit(kind);
    if (!s) return;
    s->affinity = std::min(1.0f, s->affinity + 0.06f);
    // 好感度达标后显形（彩色光晕）
...
float EchoEngine::spiritAffinity(SpiritKind kind) const
{
    for (const auto& s : m_spirits) if (s.kind == kind) return s.affinity;
    return 0.0f;
}

bool EchoEngine::isSpiritBacklashing(SpiritKind kind) const
{
    return spiritAffinity(kind) >= 0.85f;
}

QString EchoEngine::spiritBacklashText(SpiritKind kind) const
{
    switch (kind) {
    case SpiritKind::Lie:
...
void EchoEngine::accumulateDeviation(float delta)
{
...
void EchoEngine::collectSoulFragment(int id)
{
...
int EchoEngine::collectedSoulFragmentCount() const
{
    int c = 0;
...
bool EchoEngine::allSoulFragmentsCollected() const
{
    return collectedSoulFragmentCount() == static_cast<int>(m_soulFragments.size());
}

QString EchoEngine::releaseSpirit(SpiritKind kind)
{
    auto* s = findSpirit(kind);
    if (!s) return "";
    QString name = s->name;
    s->manifested = false;
    s->affinity = 0.0f;
    // 释放声灵 = 逆向播放时割舍声音 → 记录献祭，累计偏差能量，收集碎片
