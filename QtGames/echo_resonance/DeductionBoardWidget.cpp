...
void DeductionBoardWidget::paintGL()
{
    ...
    renderProposition();
    renderSpirits();       // 声灵光晕
    renderDeviation();     // 原初嗡鸣偏差能量
    renderSoulFragments(); // 灵魂碎片
...
void DeductionBoardWidget::renderSpirits()
{
    const auto& spirits = m_engine.spirits();
    // 显形的声灵在屏幕边缘渲染彩色光晕
    int manifestedCount = 0;
    for (const auto& sp : spirits) {
        if (!sp.manifested) continue;
        // 光晕：屏幕右侧边缘扩散的彩色同心圆
        float baseY = 0.85f - manifestedCount * 0.25f;
        float pulse = 0.5f + 0.3f * sinf(m_gameTime * 2.0f + manifestedCount);
        for (int ring = 0; ring < 3; ++ring) {
            float rr = 0.05f + ring * 0.03f + 0.01f * pulse;
            QColor c = sp.color();
            c.setAlpha(int(120 - ring * 35));
            drawCircleGL(0.92f, baseY, rr, c, 32);
        }
        drawTextGL(0.88f, baseY - 0.04f, sp.name, sp.color(), 0.3f);
        drawTextGL(0.88f, baseY - 0.09f, QString("好感 %1%").arg(int(sp.affinity * 100)),
                   sp.color(), 0.28f);
        // 反噬提示
        if (m_engine.isSpiritBacklashing(sp.kind)) {
            float a = 0.5f + 0.3f * sinf(m_gameTime * 4.0f);
            drawTextGL(0.88f, baseY - 0.14f, "⚠ 声灵反噬中", QColor(255, 100, 100, int(a * 255)), 0.28f);
        }
        manifestedCount++;
    }
}
...
void DeductionBoardWidget::renderSoulFragments()
{
    const auto& frags = m_engine.soulFragments();
    int total = static_cast<int>(frags.size());
    int collected = m_engine.collectedSoulFragmentCount();
    if (total <= 0) return;

    drawTextGL(-0.95f, -0.55f, QString("灵魂碎片 %1/%2").arg(collected).arg(total),
               QColor(255, 220, 120, 200), 0.4f);
    // 碎片排成一排（发光几何体）
    float startX = -0.95f, pieceW = 0.06f, gap = 0.02f;
    for (int i = 0; i < total; ++i) {
        float x = startX + i * (pieceW + gap);
        QColor c = frags[i].collected ? QColor(255, 220, 120, 220) : QColor(60, 70, 90, 120);
        if (frags[i].glowing) {
            float pulse = 0.6f + 0.4f * sinf(m_gameTime * 3.0f + i);
            c.setAlpha(int(c.alpha() * pulse));
        }
        drawRectGL(x, -0.62f, pieceW, 0.06f, c, true);
    }
    // 集齐提示
    if (m_engine.allSoulFragmentsCollected()) {
        float a = 0.5f + 0.4f * sinf(m_gameTime * 2.0f);
        drawTextGL(-0.95f, -0.72f, "碎片拼合成一张脸——那是林薇母亲的最终模样。",
                   QColor(255, 230, 150, int(a * 255)), 0.35f);
    }
}
...
void DeductionBoardWidget::renderDeviation()
{
    const auto& dev = m_engine.deviation();
    drawTextGL(0.5f, -0.78f, QString("偏差能量 %1%").arg(int(dev.amount * 100)),
               QColor(180, 160, 255, 180), 0.35f);
    if (dev.overloaded) {
        float a = 0.5f + 0.5f * sinf(m_gameTime * 1.5f);
        drawTextGL(0.5f, -0.85f, "「你听，世界正在听你。」", QColor(255, 255, 255, int(a * 220)), 0.5f);
    }
}
...
void DeductionBoardWidget::performDeduction()
{
    ...
    // 偏差能量累计（原初嗡鸣倒计时）
    m_engine.accumulateDeviation(0.05f);
...
void DeductionBoardWidget::keyPressEvent(QKeyEvent* e)
{
    ...
    case Qt::Key_C:
        // 缔结声灵契约（第三章）：5=镜 6=默 7=谎
        // 仅提示，具体缔结用 5/6/7 键
        break;
    case Qt::Key_5:
        if (m_chapter == 3) m_engine.makeContract(ContractSpirit::Mirror);
        break;
    case Qt::Key_6:
        if (m_chapter == 3) m_engine.makeContract(ContractSpirit::Silence);
        break;
    case Qt::Key_7:
        if (m_chapter == 3) m_engine.makeContract(ContractSpirit::Lie);
        break;
    case Qt::Key_R:
        // 释放声灵（终章牺牲供品）：释放当前选中样本关联的声灵
        if (m_chapter == 4 && m_selectedSample >= 0 && m_selectedSample < int(m_engine.samples().size())) {
            SpiritKind sk = m_engine.samples()[m_selectedSample].spiritKind;
            m_engine.releaseSpirit(sk);
        }
        break;
    }
    update();
