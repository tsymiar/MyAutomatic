...
type_thread_func monitor(void* arg)
{
    ...
    do {
        ...
        memset(&g_usrMsg, 0, sizeof(g_usrMsg));
        ...
        if (qq != flg && flg != 0) {
            ...
            memcpy(sd_bufs, &g_usrMsg, 2); // 2: head bytes
            if ((g_usrMsg.head == 0) && (g_usrMsg.uiCmdMsg == 0)) {
                // user.usr: 8; user.psw: 32.
                if (g_usrMsg.chk[0] == 0xee) {
                    fprintf(stdout, ">>> this message ignored.\n");
                    continue;
                }
                val_rtn = new_user(g_usrMsg.usr, g_usrMsg.psw);
                snprintf(sd_bufs + 2, 8, "%x", NE_VAL(val_rtn + 1));
                if (val_rtn == -1) {
                    snprintf(sd_bufs + offset, 37, "New user: %s", g_usrMsg.usr);
                    join_zone(0, g_usrMsg.usr, const_cast<char*>("all"));
                ...
                continue;
            } else if ((g_usrMsg.head == 0) && (g_usrMsg.uiCmdMsg == 0x1)) {
                val_rtn = user_auth(g_usrMsg.usr, g_usrMsg.psw);
                snprintf(sd_bufs + 2, 8, "%x", NE_VAL(val_rtn));
                if (val_rtn == (loggedIn = 1)) {
                    snprintf(sd_bufs + offset, 54, "[%s] logging on successfully.", g_usrMsg.usr);
                    set_user_peer(g_usrMsg.usr, IP, PORT, cur_sock);
                    Network sock;
                    static_cast<void>(memcpy((void*)(sock.ip), IP, INET_ADDRSTRLEN)),
                        static_cast<void>(sock.port = PORT),
                        sock.socket = cur_sock;
                    set_user_line(g_usrMsg.usr, sock);
                    memcpy(userName, g_usrMsg.usr, FiledSize);
                    send(cur_sock, sd_bufs, 64, 0);
                } else if (val_rtn == 0) {
                    if (memcmp(g_usrMsg.usr, userName, FiledSize) == 0) {
                        snprintf(sd_bufs + 2, 8, "%x", NE_VAL(-3));
                        snprintf(sd_bufs + offset, 57, "Warn: another [%s] been on-line.", g_usrMsg.usr);
            ...
            } else if ((g_usrMsg.head == 0) && (g_usrMsg.uiCmdMsg == 0x3)) {
            ...
            while (loggedIn) {
                ...
                if (g_usrMsg.head == 0) {
                    switch (sd_bufs[1] = g_usrMsg.uiCmdMsg) {
                    case CHECK:
                    ...
                    case LOGOUT:
                    {
                        ...
                        fprintf(stderr, "### [%0x, %x]:%s %s\n", sd_bufs[1],
                            static_cast<unsigned>(*g_usrMsg.chk), sn_stat < 0 ? strerror(errno) : "", sd_bufs + offset);
                        continue;
                    }
                    case PASSWD:
                    {
                        val_rtn = get_user_seq(g_usrMsg.usr);
                        snprintf(sd_bufs + 2, 8, "%x", NE_VAL(val_rtn));
                        if (1 == user_auth(g_usrMsg.usr, g_usrMsg.psw) && g_usrMsg.npsw[0] != '\0')
                            memcpy(users[val_rtn].psw, g_usrMsg.npsw, FiledSize);
                        else
                    ...
                    {
                        unsigned int uiIP = 0;
                        memcpy((sd_bufs + offset), g_usrMsg.usr, FiledSize);
                        val_rtn = get_user_seq(g_usrMsg.peer);
                        if (val_rtn >= 0) {
                            if (user_is_line(g_usrMsg.peer) >= 0) {
                                fprintf(stdout, "``` '%s' wants to P2P with '%s'\n", g_usrMsg.usr, g_usrMsg.peer);
                                Network p2pnt = queryNetworkParams(val_rtn);
#if !defined _WIN32
                                queryNetworkParams(g_usrMsg.peer, p2pnt);
#endif
                                ...
                                if (memcmp(g_usrMsg.chk, "P2P", 4) == 0) { // received a hole digging message
                                    fprintf(stdout, "'%s' is communicating with '%s' via P2P.\n", g_usrMsg.usr, g_usrMsg.peer);
                                    snprintf(sd_bufs + 2, 8, "%x", NE_VAL(0));
                                    memcpy((sd_bufs + 4), "P2P", 4);
                                    snprintf((sd_bufs + 64), 43, "%s request a hole.", g_usrMsg.usr);
                            ...
                            val_rtn = -3;
                            snprintf((sd_bufs + 32), 36 + FiledSize, "Get user network: No such user(%s)!", g_usrMsg.peer);
                        }
                        snprintf(sd_bufs + 2, 8, "%x", NE_VAL(val_rtn));
                    ...
                    {
                        memcpy((sd_bufs + offset), g_usrMsg.usr, FiledSize);
                        if (memcmp(g_usrMsg.chk, "NDT", 4) == 0) {
                        ...
                        if (val_rtn >= 0) {
                            if (user_is_line(g_usrMsg.peer) >= 0) {
...
#if !defined _WIN32
                                queryNetworkParams(g_usrMsg.peer, ndt_net);
#endif
                                if (ndt_net.ip[0] == '\0' || ndt_net.port == 0) {
                                    ...
                                    char ndt_msg[32];
                                    memset(ndt_msg, 0, 32);
                                    memcpy(ndt_msg, &g_usrMsg, 4);
                                    snprintf(ndt_msg + 4, 8, "%c", random);
                                    memcpy(reinterpret_cast<char*>(&g_usrMsg.status), "200", 4);
                                    memcpy(ndt_msg + offset, &g_usrMsg.peer_msg.msg, FiledSize);
                                    fprintf(stdout, "Command(%d) NDT is: %08x.\n", *(uint16_t*)g_usrMsg.peer_msg.cmd, *(uint32_t*)g_usrMsg.peer_msg.val);
                                    if (-1 != send(ndt_net.socket, ndt_msg, 32, 0)) {
                                        if (ndt_net.socket == cur_sock) {
                                            snprintf((sd_bufs + 32), 29, "%s", "Socket descriptor conflicts!");
                                            total = 64;
                                        } else {
                                            snprintf(sd_bufs + 32, 49, "[%c] NDT success to %s.", random, g_usrMsg.peer);
                                            total = 80;
                                        }
                                    } else {
...
#define _0_ ""
                    ...
                    case MEMBERS:
                    {
                        val_rtn = find_zone(g_usrMsg.seek);
                        snprintf(sd_bufs + 2, 8, "%x", NE_VAL(val_rtn));
                        if (val_rtn < 0) {
                            snprintf((sd_bufs + offset), 44, "No such NAME [%s]!", g_usrMsg.seek);
                            total = 52;
                        } else {
                    ...
                    case HOST_ZONE:
                    {
                        ...
                        if (val_rtn == -1) {
                            ...
                            int stat = join_zone(((val_rtn + 3 + MAX_ZONES)), g_usrMsg.usr, reinterpret_cast<char*>(g_usrMsg.join));
                            if (stat >= 0 && g_usrMsg.host[0] != '\0' && g_usrMsg.usr[0] != '\0') {
                                snprintf((sd_bufs + offset), 30 + FiledSize * 2, "'%s' exist, %s just joins it.", g_usrMsg.host, g_usrMsg.usr);
                                total = 87;
                            } else if (stat == -3) {
                                total = 40;
                                if (g_usrMsg.join[0] == '\0') {
                            ...
                            snprintf((sd_bufs + offset), 52, "Created group '%s' as host.", g_usrMsg.host);
                            total = 62;
                        }
                        *length = total;
                    } break;
                    case JOIN_ZONE:
                    {
                        val_rtn = join_zone(find_zone(g_usrMsg.join), g_usrMsg.usr, reinterpret_cast<char*>(g_usrMsg.join));
                        snprintf(sd_bufs + 2, 8, "%x", NE_VAL(val_rtn));
                        if (val_rtn < 0) {
                            total = 48;
                            switch (val_rtn) {
                            case -1:
                                snprintf((sd_bufs + offset), 58, "Can't find such named-group '%s'!", g_usrMsg.join);
                                total += 10;
                                break;
                            case -2:
                                snprintf((sd_bufs + offset), 51, "UserName is invalid: [%s].", g_usrMsg.usr);
                                total += 10;
                                break;
                            case -3:
                            ...
                            snprintf((sd_bufs + offset), 52, "Joining group [%s] success.", g_usrMsg.join);
                            total = 60;
                        }
                        *length = total;
                    } break;
                    case EXIT_ZONE:
                    {
                        val_rtn = exit_zone(find_zone(g_usrMsg.seek), g_usrMsg);
                        snprintf(sd_bufs + 2, 8, "%x", NE_VAL(val_rtn));
                        *length = total = 45;
                        if (val_rtn >= 0)
                    ...
                    case ALL_ZONE:
                    {
                        val_rtn = free_zone(find_zone(g_usrMsg.seek), g_usrMsg.usr);
                        snprintf(sd_bufs + 2, 8, "%x", NE_VAL(val_rtn));
                        for (int i = 0; i < MAX_ACTIVE; i++)
                            set_n_get_mem(&active[i], i);
                        if (val_rtn < 0) {
                            if (-1 != user_is_line(g_usrMsg.usr)) {
                                snprintf(sd_bufs + 3, 9, "%x", -1);
                                memcpy((sd_bufs + offset), g_usrMsg.usr, FiledSize);
                                memcpy((sd_bufs + 32), g_usrMsg.sign, FiledSize);
                    ...
                    int j = 0;
                    for (c = 2; c < static_cast<int>(sizeof(g_usrMsg)); c++) {
                        if ((c - 32 > 0) && ((c - 32) % 8 == 0)
...
comm_err0:
    {
        for (int i = 0; i < MAX_ACTIVE; i++)
            set_n_get_mem(&active[i], i);
        memset(active[user_is_line(g_usrMsg.usr)].user, 0, FiledSize);
    }
#if !defined _WIN32
    comm_err1 : {
        g_threadNo_--;
...
void func_waitpid(int signo)
{
    ...
    while ((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
        ...
        while (total >= 0 && total < expect) {
            ...
            // total ∈ [0, expect), remain = expect - total
