...
{	soap_free_temp(soap);
	if (p)
	{	if (soap_begin_send(soap) || (soap_serialize_api__trans(soap, p), 0) || soap_put_api__trans(soap, p, "api:trans", NULL) || soap_end_send(soap))
			return soap->error;
	}
	return SOAP_OK;
}
...
{	soap_free_temp(soap);
	if (p)
	{	if (soap_begin_send(soap) || (soap_serialize_api__transResponse(soap, p), 0) || soap_put_api__transResponse(soap, p, "api:transResponse", NULL) || soap_end_send(soap))
			return soap->error;
	}
	return SOAP_OK;
}
