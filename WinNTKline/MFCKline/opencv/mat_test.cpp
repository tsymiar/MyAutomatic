...
  g_srcImage=imread("image0.jpg",CV_LOAD_IMAGE_ANYDEPTH | CV_LOAD_IMAGE_ANYCOLOR);  
  if(!g_srcImage.data ) 
  {   
...
  Mat mat(480, 480, CV_8UC4);
  ...
  try{    
    imwrite("image0.png", mat, compression_params); 
...
  
  Mat roiImage= imread("image1.jpg");  
  Mat logoImage= imread("image0.png");
  if(!roiImage.data )  
  {
...
 
  ...
   
   ...
   Mat avgImage; 
   namedWindow("均值滤波");
   GaussianBlur(g_srcImage, avgImage, Size( 3, 3 ), 0, 0 ); 
   imshow("均值滤波" ,avgImage);
