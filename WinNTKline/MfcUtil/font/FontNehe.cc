...
namespace freetype {
	...
	// With FreeType Fonts.
	void freetype::textout(const font_data &ft_font, float x, float y, const char *fmt, ...) {
...
int freetype::DrawGLScene(GLvoid)                                     // Here's Where We Do All The Drawing
{
	freetype::font_data our_font;
	our_font.init("C:\\Windows\\Fonts\\simsun.ttc", 16);
	float cnt1 = 0;
	float cnt2 = 0;
	glColor3ub(0, 0, 0xff);
	...
	glPushMatrix();
	glLoadIdentity();
	freetype::textout(our_font, 320, 200, "Active FreeType Text - %7.2f", cnt1);
	glPopMatrix();
	our_font.clean();
