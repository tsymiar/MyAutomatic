...
bool OGLKview::DrawItems(Market market, int row)
{
    ...
    view.DrawKtext(listime, pnt, 13);
    pnt.x = 0.9f;
    sprintf_s(info, "%.2f", g_ITEM.pc_);
    view.DrawKtext(info, pnt, 13);
    pnt.x = 1.13f;
    ...
    view.DrawKtext(g_ITEM.bs, pnt, 13);
    return tmp;
}
...
void OGLKview::DrawDash(Point pnt[2])
{
    glColor3f(1, 0, 0);
...
#undef _UNICODE
    ...
    SDL_GL_init();
    if (ttffont == NULL)
        ttffont = TTF_OpenFont("../MfcUtil/font/simfang.ttf", size);
    if (ttffont == NULL)
    {
