...
$.idealforms.errors = {
  ...
  number: '必须是数字.',
  digits: '必须是唯一的数字.',
  name: '至少有3个字符长，并且仅包含字母.',
  username: '至少在32之间和4个字符长，并以字母开头。(.)',
  passwd: '至少6个字符长，并且至少包含一个数字，一个大写字母和一个小写字母.',
  strongpass: '至少为8个字符长，至少包含一个大写字母和一个小写字母和一个数字或特殊字符.',
...
var getFilters = function() {
  ...
  var filters = {
    ...
    passwd: {
      regex: /(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}/,
      error: $.idealforms.errors.passwd
    },
...
function showResp(){
    ...
    xmlhttp.open("POST","./trans/service.php?action=submit_array",true);
    ...
    xmlhttp.onreadystatechange = function()
    {
        ...
        {
            document.getElementById("dontjump").innerHTML=xmlhttp.responseText;
        }
    };
}